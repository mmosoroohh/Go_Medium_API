...
func TestMain(m *testing.M) {
	...
	if err != nil {
		log.Fatalf("Error while getting env %v\n", err)
	}
	Database()
	os.Exit(m.Run())
}
...
func Database() {
	...
	if TestDbDriver == "mysql" {
		...
		if err != nil {
			fmt.Printf("Can't connect to %s database\n", TestDbDriver)
	...
	if TestDbDriver == "postgres" {
		DBURL := fmt.Sprintf("host=%s port=%s user=%s dbname=%s sslmode password=%s", os.Getenv("TestDbHost"), os.Getenv("TestDbPort"), os.Getenv("TestDbUser"), os.Getenv("TestDbName"), os.Getenv("TestDbPassword"))
		server.DB, err = gorm.Open(TestDbDriver, DBURL)
		if err != nil {
			fmt.Printf("Can't connect to %s database\n", TestDbDriver)
			log.Fatal("This is the error: ", err)
...
func seedOneUserAndOnePost() (models.Post, error) {
	...
	post := models.Post{
		Title:    "This is the title john",
		Content:  "This is the content john",
		AuthorID: user.ID,
	}
...
func seedUsersAndPosts() ([]models.User, []models.Post, error) {
	...
	var users = []models.User{
		models.User{
			Username: "Carolyne Osoro",
			Email:    "caro.osoro@gmail.com",
			Password: "Password",
		},
		models.User{
			Username: "Rafeal Osoro",
			Email:    "rafeal.osoro@gmail.com",
			Password: "Password",
		},
	}
